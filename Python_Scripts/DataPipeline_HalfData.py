...
def allDatatoDataframe(FolderNavigation,DataFolders):
    ...
    for idx1,folder in enumerate(DataFolders):
        ...
        for idx2,file in enumerate(files):
            df_temp = pd.read_csv(FolderNavigation+folder+'/'+file)
...
def gestureMagnitudeShifting(training,accel_sets, fract):
    ...
    for idx1, aset in enumerate(accel_sets):
        for molecule, denominator in fract:
            magSeries = pd.Series(data={'gesture': training['gesture'][idx1], 'acceleration':(np.array(aset, dtype=np.float32) * molecule / denominator).tolist()})
    ...
    for i in range(int(len(data) / molecule) - 1):
        for j in range(len(data[i])):
            for k in range(denominator):
                tmp_data[denominator * i + k][j] = (data[molecule * i + k][j] * (denominator - k) + data[molecule * i + k + 1][j] * k) / denominator
    return tmp_data
...
def gestureStretchShrink(train_set, accel_sets, fract):
    ...
    for idx1, aset in enumerate(accel_sets):
        ...
        for molecule, denominator in fract:
            ...
            timeSeries = pd.Series(data={'gesture': train_set['gesture'][idx1], 'acceleration':shiftedAccels})
...
def gestureWithNoise(train_set,accel_sets):
    ...
    for idx1, aset in enumerate(accel_sets):
        for t in range(5):
            ...
            noiseSeries = pd.Series(data={'gesture': train_set['gesture'][idx1], 'acceleration':tmp_data})  
...
def gestureTimeShift(train_set, accel_sets):
    ...
    for idx1, aset in enumerate(accel_sets):
        for i in range(5):
            shiftSeries = pd.Series(data={'gesture': train_set['gesture'][idx1], 'acceleration':(np.array(aset, dtype=np.float32)+ ((random.random()- 0.5)*50)).tolist()})
...
def dataToLength(data_set,seq_length,dim):
    ...
    for idx4, proacc in enumerate(proc_acc):
        ...
        for half in pad_acc:
            padSeries = pd.Series(data={'gesture': data_set['gesture'][idx4], 'acceleration': half.tolist()})
...
if __name__=='__main__':
    ...
    gest_id = {'Single_Wave_Half': 0, 'Fist_Pump_Half': 1, 'Random_Motion_Half': 2, 'Speed_Mode_Half': 3, 'Pumped_Up_Half':4}
    folders = ["Fist_Pump_Half","Single_Wave_Half","Speed_Mode_Half","Random_Motion_Half", "Pumped_Up_Half"]
    ...
    print('len of trainingData Speed mode: '+ str(len(trainingData.query('gesture == "Speed_Mode_Half"'))))
    print('len of validationData Speed mode: '+ str(len(validationData.query('gesture == "Speed_Mode_Half"'))))
    print('len of testingData Speed mode: '+ str(len(testingData.query('gesture == "Speed_Mode_Half"'))))

    print('len of trainingData fist_pump: '+ str(len(trainingData.query('gesture == "Fist_Pump_Half"'))))
    print('len of validationData fist_pump: '+ str(len(validationData.query('gesture == "Fist_Pump_Half"'))))
    print('len of testingData fist_pump: '+ str(len(testingData.query('gesture == "Fist_Pump_Half"'))))

    print('len of trainingData single_wave: '+ str(len(trainingData.query('gesture == "Single_Wave_Half"'))))
    print('len of validationData single_wave: '+ str(len(validationData.query('gesture == "Single_Wave_Half"'))))
    print('len of testingData single_wave: '+ str(len(testingData.query('gesture == "Single_Wave_Half"'))))

    print('len of trainingData random_motion: '+ str(len(trainingData.query('gesture == "Random_Motion_Half"'))))
    print('len of validationData random_motion: '+ str(len(validationData.query('gesture == "Random_Motion_Half"'))))
    print('len of testingData random_motion: '+ str(len(testingData.query('gesture == "Random_Motion_Half"'))))

    print('len of trainingData random_motion: '+ str(len(trainingData.query('gesture == "Pumped_Up_Half"'))))
    print('len of validationData random_motion: '+ str(len(validationData.query('gesture == "Pumped_Up_Half"'))))
    print('len of testingData random_motion: '+ str(len(testingData.query('gesture == "Pumped_Up_Half"'))))
    

    trainingData.to_csv('train_set_half.csv', index=False)
    validationData.to_csv('val_set_half.csv', index=False)
    testingData.to_csv('test_set_half.csv', index=False)
    train_set = pd.read_csv('train_set_half.csv',converters={'acceleration': eval})
    val_set = pd.read_csv('val_set_half.csv',converters={'acceleration': eval})
    test_set = pd.read_csv('test_set_half.csv',converters={'acceleration': eval})

    #Data Augmenting
    training_accelerations = train_set['acceleration'].to_numpy()
    shiftingFractions=[(3, 2), (5, 3), (2, 3), (3, 4), (9, 5), (6, 5), (4, 5)]
    train_Mag_Data = gestureMagnitudeShifting(train_set, training_accelerations, shiftingFractions)
    train_TimeSS_data = gestureStretchShrink(train_set, training_accelerations, shiftingFractions)
    train_Noise_data = gestureWithNoise(train_set, training_accelerations)
    train_Shift_data = gestureTimeShift(train_set, training_accelerations)

    #combine all the Data sets into one
    processedTrain_set = pd.DataFrame(columns=['gesture','acceleration'])
    processedTrain_set = pd.concat([train_set,train_Mag_Data, train_TimeSS_data, train_Noise_data, train_Shift_data], ignore_index=True)

    #Set all data to exactly dataPointNum datapoints
    dataPointNum = 380
    train_final_data = dataToLength(processedTrain_set,dataPointNum,3)
    val_final_data = dataToLength(val_set,dataPointNum,3)
    test_final_data = dataToLength(test_set,dataPointNum,3)
    ...
    val_final_data.to_csv('processed_val_set_half.csv', index=False)
    test_final_data.to_csv('processed_test_set_half.csv', index=False)
    train_final_data.to_csv('processed_train_set_half.csv', index=False)